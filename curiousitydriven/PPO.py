...
class PPOAgent:
    def __init__(self, env_name):
        self.env = set_env.setup_env(env_name)
        self.n_actions = self.env.action_space.n
        self.state_size = np.array([240 * 256 * 3, ])
        self.lr = 1e-4  # 0.0001
        self.EPISODES = 10000
        self.eps = 0
        self.epochs = 0
    ...
    def replay(self, states, actions, rewards, predictions, dones, next_states):
        ...
        a_loss = self.actor.actor.fit(states, y_true, epochs=self.epochs, verbose=0, shuffle=self.shuffle, callbacks=[
            ModelCheckpoint(filepath="curiousitydrivenMario/checkpointActor", save_weights_only=True,
                            save_best_only=True, save_freq=5, verbose=1)])
        c_loss = self.critic.critic.fit([states, values], target, epochs=self.epochs, verbose=0, shuffle=self.shuffle,
                                        callbacks=[
                                            ModelCheckpoint(filepath="curiousitydrivenMario/checkpointCritic",
    ...
    def run(self):
        ...
        while True:
            ...
            while not done:
                ...
                states.append(state)
                next_states.append(
                    np.reshape(next_state, [1, next_state.shape[0] * next_state.shape[1] * next_state.shape[2]]))
                actions.append(action)
                rewards.append(reward)
                dones.append(done)
                actions_dist.append(prediction)
                mean_entropy.append(entropy)
                state = np.reshape(next_state, [1, next_state.shape[0] * next_state.shape[1] * next_state.shape[2]])
                score += reward
    ...
    def test(self, episodes=100):
        ...
        for e in range(episodes):
            state = self.env.reset()
            state = state = np.reshape(state, [1, state.shape[0] * state.shape[1] * state.shape[2]])
            done = False
            score = 0
            while not done:
                self.env.render()
...
if __name__ == "__main__":
    ...
    agent.run()
    # agent.test()
    # env = set_env.setup_env(env_name)
